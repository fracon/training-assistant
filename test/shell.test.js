...
test('loadAppVersion resolves the packaged version and degrades to null on any failure', async () => {
  ...
  assert.equal(require('../package.json').version, '0.6.3');
  assert.equal(await loadAppVersion(ok({ version: '0.6.3' })), '0.6.3');
  assert.equal(await loadAppVersion(async () => ({ ok: false, json: async () => ({ version: '0.6.3' }) })), null);
